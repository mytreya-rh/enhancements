...
`istio-csr` components within the cluster environment, and facilitating operations like monitoring and resource discovery.
...
type IstioCSRSpec struct {
	...
	// cleanupOnDeletion is for indicating operator to remove all the resources created
	// for istio-csr agent installation including the cert-manager `Certificate`
	// resource created for obtaining certificate for istiod from configured issuer.
...
type IstioCSRConfig struct {
	...
	// istio-csr creates a configmap named `istio-ca-root-cert` containing the root CA certificate, which the
	// istio data plane uses to verify server certificates. Its default behavior is to create and monitor the configmaps
	// in all namespaces. The istioDataPlaneNamespaceSelector helps restrict which namespaces the configmap is created
	// in by using label selectors, such as `maistra.io/member-of=istio-system`, which also is attached to all the desired
	// namespaces that are part of the data plane.
...
type IstiodTLSConfig struct {
	...
	// +kubebuilder:validation:Enum:=2048;256;384
...
type ConfigMapReference struct {
	...
	// namespace in which the configmap exists. If empty, configmap will be looked up in operator namespace.
	// +kubebuilder:validation:MinLength:=1
	// +kubebuilder:validation:MaxLength:=63
...
- When user configures `spec.istioCSRConfig.certManager.istioCACertificate` in `istiocsrs.operator.openshift.io`, user should
  manage keeping the configmap updated when the CA certificate is rotated. Failing to do so would cause the communication
  breakdown in the mesh, and would degrade functionalities of multiple services part of the mesh.
